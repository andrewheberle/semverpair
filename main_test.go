...
func Test_parsePositionalArgs(t *testing.T) {
	tests := []struct {
		...
		{"both set with no args", []string{}, "a", "b", "a", "b"},
		{"both set with one arg", []string{"other"}, "a", "b", "a", "b"},
		{"both set with two args", []string{"other", "again"}, "a", "b", "a", "b"},
		{"both  set with three args", []string{"other", "again", "ignored"}, "a", "b", "a", "b"},
		{"none set with no args", []string{}, "", "", "", ""},
		{"none set with one args", []string{"first"}, "", "", "first", ""},
		{"none set with two args", []string{"first", "second"}, "", "", "first", "second"},
		{"none set with three args", []string{"first", "second", "ignored"}, "", "", "first", "second"},
		{"first set with no args", []string{}, "firstset", "", "firstset", ""},
		{"first set with one args", []string{"second"}, "firstset", "", "firstset", "second"},
		{"first set with two args", []string{"second", "ignored"}, "firstset", "", "firstset", "second"},
		{"first set with three args", []string{"second", "ignored1", "ignored2"}, "firstset", "", "firstset", "second"},
		{"second set with no args", []string{}, "", "secondset", "", "secondset"},
		{"second set with one args", []string{"first"}, "", "secondset", "first", "secondset"},
		{"second set with two args", []string{"first", "ignored"}, "", "secondset", "first", "secondset"},
		{"second set with three args", []string{"first", "ignored1", "ignored2"}, "", "secondset", "first", "secondset"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, got2 := parsePositionalArgs(tt.args, tt.first, tt.second)
			if tt.want != got {
				t.Errorf("parsePositionalArgs() = %v, want %v", got, tt.want)
			}
			if tt.want2 != got2 {
				t.Errorf("parsePositionalArgs() = %v, want %v", got2, tt.want2)
			}
		})
	}
}
